...
//            System.out.println(Integer.parseInt(empleados[i][3]));         
        ...
        Incidencia temporal = new Incidencia(
                "Falla en sistema de frenos",
                parque.getListaZonas().get(1).getListaAtracciones().get(2), //Aventura Encantada
                "PENDIENTE",
                parque.getListaEmpleados().get(0) // María López
        );

     
        new Incidencia(
                "Cortocircuito en iluminación",
                parque.getListaZonas().get(1).getListaAtracciones().get(2), //Aventura Encantada
                "EN PROCESO",
                parque.getListaEmpleados().get(5) // Miguel Ángel Castro
        );

        new Incidencia(
                "Figura desgastada en carrusel",
                parque.getListaZonas().get(1).getListaAtracciones().get(2), //Aventura Encantada
                "PENDIENTE",
                parque.getListaEmpleados().get(1) // Carlos Ruiz
        );

        new Incidencia(
                "Sistema de control no responde",
                parque.getListaZonas().get(1).getListaAtracciones().get(2), //Aventura Encantada
                "EN PROCESO",
                parque.getListaEmpleados().get(0) // María López
        );
        ...
        new Incidencia(
                ...
                parque.getListaZonas().get(1).getListaAtracciones().get(2), //Aventura Encantada
                "RESUELTA",
                parque.getListaEmpleados().get(3) // Pedro Gómez
        );

        new Incidencia(
                "Sistema de bloqueo defectuoso",
                parque.getListaZonas().get(1).getListaAtracciones().get(2), //Aventura Encantada
                "PENDIENTE",
                parque.getListaEmpleados().get(6) // Elena Vargas
        );
